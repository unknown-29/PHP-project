<?php
    ...
    if($pwd===$cpwd and preg_match($patternsp, $pwd)and preg_match($patternd, $pwd)and preg_match($patternsm, $pwd)and preg_match($patterncap, $pwd) and strlen($pwd)>8){
        ...
        header("Location:forgotPassword.php?error=Password need to be more than 8 characters and should have atleast one small case character, one upper case character, one digit and one special character.");
